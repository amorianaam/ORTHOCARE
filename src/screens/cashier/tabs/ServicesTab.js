...
const InvoicePanel = ({ visitId, visitData, token, onClose, onPaid, onPatientClick }) => {
  ...
  const totalUnpaid = unpaidLabs.reduce((sum, r) => sum + parseFloat(r.price || 0), 0) + 
                      unpaidRads.reduce((sum, r) => sum + (radFilm[r.id] !== false ? parseFloat(r.price_with_film || 0) : parseFloat(r.price_without_film || 0)), 0);
