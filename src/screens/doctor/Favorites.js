...
import {
  ...
  Pill, Heart, Award, Check, X, Settings, FolderHeart, Info, Activity, RefreshCcw, Edit2, Maximize2, Minimize2, Grid, List
} from 'lucide-react';
...
export default function DoctorFavorites() {
  ...
  const handleDeleteFavMed = async (id) => {
    if (!window.confirm('هل أنت متأكد من إزالة هذا الدواء من المفضلة؟')) return;
    try {
      await axios.delete(`/api/doctor/favorites/medications/${id}`, { headers });
      toast.success('تمت إزالة الدواء بنجاح');
      setFavoriteMeds(prev => prev.filter(m => m.id !== id));
    } catch {
      toast.error('خطأ في إزالة الدواء');
    }
  };
  ...
  const handleEditFavMed = (item) => {
    setMedData({
      ...
      dosage: item.dosage || '',
      frequency: '', // not stored separately
  ...
  // ─── CRUD Favorite Tests ─────────────────────────────────────────
  const handleDeleteFavTest = async (item) => {
    if (!window.confirm('هل أنت متأكد من إزالة هذا الفحص من المفضلة؟')) return;
    try {
      await axios.delete(`/api/doctor/favorites/${item.test_type}/${item.id}`, { headers });
  ...
  const handleAddFavMedSubmit = async (e) => {
    ...
    // The backend uses ON DUPLICATE KEY UPDATE for medicationName
    // We concatenate frequency into instructions since backend only stores dosage, duration, instructions
    const finalInstructions = medData.frequency ? `${medData.frequency} - ${medData.instructions}` : medData.instructions;
  ...
  const handleAddFavTestSubmit = async (e) => {
    ...
    try {
      if (editingFavTestId) {
        await axios.delete(`/api/doctor/favorites/${selectedFavTestItem.test_type}/${editingFavTestId}`, { headers });
      }
  ...
  const handleDeleteBundle = async (id) => {
    if (!window.confirm('هل أنت متأكد من حذف هذه الحزمة المجمّعة؟')) return;
    try {
      await axios.delete(`/api/doctor/favorites/bundles/${id}`, { headers });
      toast.success('تم حذف الحزمة الطبية بنجاح');
      setBundles(prev => prev.filter(b => b.id !== id));
    } catch {
      toast.error('خطأ في حذف الحزمة');
    }
  };
    ...
    <div className="space-y-6" dir="rtl">
                      ...
                      medsViewMode === 'grid' ? (
                        <div key={item.id} className="bg-white p-4 rounded-2xl border border-gray-150 space-y-2 relative shadow-luxury hover-lift transition-all duration-300">
                          ...
                          <div className="text-[10px] text-gray-500 font-bold space-y-1.5 mt-3 pt-3 border-t border-gray-50">
                            {item.dosage && <p className="flex items-center gap-1.5"><span className="text-indigo-400">💊 الجرعة:</span> {item.dosage}</p>}
                            {item.frequency && <p className="flex items-center gap-1.5"><span className="text-indigo-400">🔄 التكرار:</span> {item.frequency}</p>}
                            {item.duration && <p className="flex items-center gap-1.5"><span className="text-indigo-400">⏱️ المدة:</span> {item.duration}</p>}
                            {item.instructions && <p className="flex items-center gap-1.5"><span className="text-indigo-400">✍️ تعليمات:</span> {item.instructions}</p>}
                          </div>
                        </div>
                      ) : (
                        <div key={item.id} className="flex flex-col md:flex-row md:items-center justify-between bg-white p-3.5 rounded-xl border border-gray-150 hover:bg-slate-50 transition-all duration-300 shadow-sm gap-4">
                          <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-2 text-[10px] font-bold">
                              {item.dosage && <span className="bg-indigo-50/70 border border-indigo-100 text-indigo-700 px-2.5 py-1 rounded-md">💊 الجرعة: {item.dosage}</span>}
                              {item.frequency && <span className="bg-emerald-50/70 border border-emerald-100 text-emerald-700 px-2.5 py-1 rounded-md">🔄 التكرار: {item.frequency}</span>}
                              {item.duration && <span className="bg-amber-50/70 border border-amber-100 text-amber-700 px-2.5 py-1 rounded-md">⏱️ المدة: {item.duration}</span>}
                              {item.instructions && <span className="bg-rose-50/70 border border-rose-100 text-rose-700 px-2.5 py-1 rounded-md">✍️ ملاحظات: {item.instructions}</span>}
                            </div>
                          </div>
          ...
          {activeTab === 'tests' && (
            <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm space-y-4">
              <h3 className="font-black text-sm text-gray-800 flex items-center justify-between pb-3 border-b border-gray-50">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex bg-gray-100 rounded-lg p-0.5">
                    <button onClick={() => setTestsViewMode('grid')} className={`p-1.5 rounded-md transition-colors ${testsViewMode === 'grid' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}><Grid size={14}/></button>
                    <button onClick={() => setTestsViewMode('list')} className={`p-1.5 rounded-md transition-colors ${testsViewMode === 'list' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}><List size={14}/></button>
                  </div>
                  <button 
                  ...
                  <div className="space-y-8">
                    ...
                    {favoriteTests.filter(t => t.test_type === 'lab').length > 0 && (
                      <div>
                        ...
                        <div className={`transition-all duration-300 ${testsViewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" : "flex flex-col gap-2"}`}>
                          {favoriteTests.filter(t => t.test_type === 'lab').map(item => (
                    testsViewMode === 'grid' ? (
                      <div key={item.id} className="bg-white p-4 rounded-2xl border border-gray-150 flex flex-col justify-between shadow-luxury hover-lift transition-all duration-300 min-h-[100px] relative group">
                        ...
                        <p className="text-[10px] text-gray-400 font-bold mt-2 pt-2 border-t border-gray-50">{item.price} ريال يمني</p>
                      </div>
                    ) : (
                      <div key={item.id} className="flex items-center justify-between bg-white p-3.5 rounded-xl border border-gray-150 hover:bg-slate-50 transition-all duration-300 shadow-sm gap-4">
                        <div className="flex items-center gap-3">
                          <h4 className="font-black text-slate-800 text-sm">{item.name}</h4>
                        </div>
                        <div className="flex items-center gap-4">
                          <p className="text-[11px] text-gray-500 font-bold">{item.price} ريال يمني</p>
                          <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    {favoriteTests.filter(t => t.test_type === 'radiology').length > 0 && (
                      <div>
                        ...
                        <div className={`transition-all duration-300 ${testsViewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" : "flex flex-col gap-2"}`}>
                          {favoriteTests.filter(t => t.test_type === 'radiology').map(item => (
                    testsViewMode === 'grid' ? (
                      <div key={item.id} className="bg-white p-4 rounded-2xl border border-gray-150 flex flex-col justify-between shadow-luxury hover-lift transition-all duration-300 min-h-[100px] relative group">
                        <div className="absolute top-4 left-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleEditFavTest(item)} className="p-1.5 text-indigo-400 hover:text-indigo-600 rounded-lg hover:bg-indigo-50 transition-colors"><Edit2 size={14} /></button>
                    ...
                    {favoriteTests.filter(t => t.test_type === 'clinical').length > 0 && (
                      <div>
                        ...
                        <div className={`transition-all duration-300 ${testsViewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" : "flex flex-col gap-2"}`}>
                          {favoriteTests.filter(t => t.test_type === 'clinical').map(item => (
                    testsViewMode === 'grid' ? (
                      <div key={item.id} className="bg-white p-4 rounded-2xl border border-gray-150 flex flex-col justify-between shadow-luxury hover-lift transition-all duration-300 min-h-[100px] relative group">
                        <div className="absolute top-4 left-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleEditFavTest(item)} className="p-1.5 text-indigo-400 hover:text-indigo-600 rounded-lg hover:bg-indigo-50 transition-colors"><Edit2 size={14} /></button>
          ...
          {activeTab === 'bundles' && (
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row justify-between md:items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex bg-gray-100 rounded-lg p-0.5">
                    <button onClick={() => setBundlesViewMode('grid')} className={`p-1.5 rounded-md transition-colors ${bundlesViewMode === 'grid' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}><Grid size={14}/></button>
                    <button onClick={() => setBundlesViewMode('list')} className={`p-1.5 rounded-md transition-colors ${bundlesViewMode === 'list' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}><List size={14}/></button>
                  </div>
                  <button
              ...
              {/* Bundles list grid */}
              <div className={`transition-all duration-300 ${bundlesViewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5" : "flex flex-col gap-3"}`}>
                {bundles.length === 0 ? (
                  <div className={`${bundlesViewMode === 'grid' ? 'col-span-full' : 'w-full'} text-center py-12 text-gray-400 text-xs bg-white rounded-3xl border border-gray-100 shadow-sm`}>لم تقم بإنشاء باقات مجمعة مسبقاً.</div>
                ) : (
                  bundles.map(bundle => (
                    bundlesViewMode === 'grid' ? (
                      ...
                      <div key={bundle.id} className="bg-white rounded-2xl p-4 border border-gray-150 hover:bg-slate-50 shadow-sm transition-all duration-300 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0 border-t md:border-t-0 pt-3 md:pt-0 border-gray-100">
                          <button
                            onClick={() => handleEditBundle(bundle)}
                            className="p-2 text-indigo-400 hover:text-indigo-600 rounded-lg hover:bg-indigo-50 transition-colors"
                          >
      ...
      {isCreatingBundle && createPortal(
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm modal-overlay-anim" dir="rtl">
          <div
            className={`bg-white rounded-3xl shadow-luxury overflow-hidden flex flex-col border border-gray-100 animate-scale-in transition-all duration-300 ${isMaximized ? 'w-[98vw] h-[98vh]' : 'w-full max-w-4xl h-[80vh]'}`}
          >
              {/* Header */}
              ...
              <form onSubmit={handleCreateBundleSubmit} className="flex-1 flex flex-col overflow-hidden min-h-0">
                <div className={`flex flex-col md:flex-row flex-1 overflow-hidden ${isMaximized ? 'max-w-6xl mx-auto w-full' : ''}`}>
                {/* Catalog list selection (Left) */}
                <div className="flex-1 flex flex-col p-5 overflow-hidden border-l border-gray-100">
                  ...
                  <div className="relative mb-3 flex-shrink-0">
                    ...
                    <input
                      ...
                      className="input-base pr-9 text-xs py-2 font-semibold shadow-xs w-full"
                    />
                  </div>

                  <div className="flex-1 overflow-y-auto pr-1">
                    <div className={`grid gap-2 ${isMaximized ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-1 sm:grid-cols-2'}`}>
                          ...
                          <div
                            ...
                            className={`flex items-center gap-2.5 p-3 rounded-xl border cursor-pointer transition-all ${
                              isChecked
                                ...
                                : 'bg-white border-gray-150 hover:border-indigo-150'
                            }`}
                          >
                            <input
                              type="checkbox"
                            ...
                            />
                            <div>
                              <p className="font-extrabold text-[11px] text-gray-800">{item.name}</p>
                              <span className={`px-1.5 py-0.2 rounded text-[7px] font-black mt-1 inline-block ${item.color}`}>
                                {item.typeLabel}
                              </span>
                            </div>
                          </div>
                ...
                <div className={`flex-shrink-0 bg-gray-50/50 flex flex-col overflow-hidden p-5 space-y-5 transition-all duration-300 ${isMaximized ? 'w-full md:w-96 border-t md:border-t-0 border-gray-200' : 'w-full md:w-80'}`}>
                  <div>
                  ...
                  <div className="flex-1 overflow-y-auto space-y-2">
                    ...
                    {selectedBundleItems.length === 0 ? (
                      ...
                      selectedBundleItems.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-white p-2 rounded-lg border text-[10px] font-bold text-gray-700 shadow-xs">
      ...
      {isAddingTest && createPortal(
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm modal-overlay-anim" dir="rtl">
          <div className={`bg-white rounded-3xl shadow-luxury overflow-hidden flex flex-col border border-gray-100 animate-scale-in transition-all duration-300 ${isMaximized ? 'w-[98vw] h-[98vh]' : 'w-full max-w-4xl h-[80vh]'}`}>
            <div className="flex justify-between items-center p-5 border-b border-gray-150 flex-shrink-0 bg-gray-50/50">
              <h3 className="font-extrabold text-base text-gray-800 flex items-center gap-2">
                      ...
                      <div className={`grid gap-2 ${isMaximized ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-1 sm:grid-cols-2'}`}>
                        {(catalogs[testModalTab]?.filter(i => !testSearchTerm.trim() || i.name?.toLowerCase().includes(testSearchTerm.toLowerCase())) || []).map(item => {
                          ...
                          return (
                            ...
                            >
                              <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${isSelected ? 'border-indigo-600 bg-indigo-600' : 'border-gray-300'}`}>
                                {isSelected && <div className="w-1.5 h-1.5 bg-white rounded-full"></div>}
                              </div>
                              <div>
                                <p className="font-extrabold text-[11px] text-gray-800">{item.name}</p>
                                <span className={`px-1.5 py-0.2 rounded text-[7px] font-black mt-1 inline-block ${testModalTab === 'lab' ? 'bg-blue-50 text-blue-700' : testModalTab === 'radiology' ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'}`}>
                                  {testModalTab === 'lab' ? 'مختبر' : testModalTab === 'radiology' ? 'أشعة' : 'خدمة'}
                                </span>
                              </div>
                            </div>
                          );
                    ...
                    <div className="flex-1 overflow-y-auto">
                      ...
                      {!selectedFavTestItem ? (
                        ...
                        <div className="bg-white p-4 rounded-xl border shadow-sm space-y-3">
                          <h4 className="font-black text-indigo-900 text-sm">{selectedFavTestItem.name}</h4>
                          <div className="flex items-center gap-2">
                            <span className={`px-2 py-0.5 rounded text-[9px] font-black ${selectedFavTestItem.test_type === 'lab' ? 'bg-blue-50 text-blue-700' : selectedFavTestItem.test_type === 'radiology' ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'}`}>
                              {selectedFavTestItem.typeLabel}
                            </span>
                            <span className="text-[10px] font-bold text-gray-500">{selectedFavTestItem.price} ريال يمني</span>
                          </div>
                        </div>
                      )}
                    </div>
