...
exports.addFavoriteMed = async (req, res) => {
  try {
    ...
    await db.execute(
      `INSERT INTO doctor_favorite_meds (doctor_id, medication_name, dosage, duration, instructions) 
       VALUES (?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE dosage = VALUES(dosage), duration = VALUES(duration), instructions = VALUES(instructions)`,
      [req.user.id, medicationName.trim(), dosage, duration, instructions]
    );
