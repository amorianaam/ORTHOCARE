...
exports.getPatientHistory = async (req, res) => {
  try {
    ...
    // Fetch visits
    const [visits] = await db.execute(`
      SELECT id, visit_number, created_at, status, closed_at 
      FROM visits 
      WHERE patient_id = ? 
    ...
    for (let visit of visits) {
      ...
      const [lab] = await db.execute(`
        SELECT l.name, r.status, r.result_notes, r.result_file
      ...
      const [rad] = await db.execute(`
        SELECT rad.name, r.status, r.result_notes, r.result_file, r.with_film, r.radiology_film_id, f.film_size
...
exports.cancelServiceRequest = async (req, res) => {
  try {
    ...
    // Check if the service request is pending_payment
    const [existing] = await db.execute(
      `SELECT status, visit_id FROM ${tableName} WHERE id = ?`,
      [id]
    );
...
exports.savePrescription = async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      await conn.execute(
        `INSERT INTO prescription_items (prescription_id, medication_name, dosage, duration, instructions) VALUES (?, ?, ?, ?, ?)`,
        [prescriptionId, item.medication_name.trim(), item.dosage || '', item.duration || '', item.instructions || '']
      );
    }
