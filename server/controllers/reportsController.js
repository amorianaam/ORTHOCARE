...
exports.getDoctorAnalyticalReports = async (req, res) => {
  ...
  try {
    ...
    let completedQuery = `
      ...
      FROM visits v
      JOIN patients p ON v.patient_id = p.id
      WHERE v.status = 'completed' AND v.created_by = ?
    `;
    let completedParams = [doctorId];
    if (startDate && endDate) {
      completedQuery += ' AND v.closed_at BETWEEN ? AND ?';
      completedParams.push(startDate + ' 00:00:00', endDate + ' 23:59:59');
    }
    completedQuery += ' ORDER BY v.closed_at DESC';
    ...
    for (const v of completedRows) {
      ...
      const [rads] = await db.execute(`
        SELECT rr.id, rr.status, rr.price, rr.discount_percentage, rr.discount_amount, rr.final_price, rr.is_free, rt.name
    ...
    // 2. Discounts
    let discountsQuery = `
      SELECT 'lab' as type, lr.requested_at as date, p.full_name as patient_name, lt.name as service_name, lr.price as original_price, lr.discount_percentage, lr.discount_amount, lr.final_price
      FROM visit_lab_requests lr
    ...
    discountsQuery += `
      ...
      SELECT 'radiology' as type, rr.requested_at as date, p.full_name as patient_name, rt.name as service_name, rr.price as original_price, rr.discount_percentage, rr.discount_amount, rr.final_price
      FROM visit_radiology_requests rr
    ...
    discountsQuery += `
      ...
      SELECT 'clinical' as type, cr.created_at as date, p.full_name as patient_name, cs.name as service_name, cr.price as original_price, cr.discount_percentage, cr.discount_amount, cr.final_price
      FROM visit_clinical_service_requests cr
    ...
    // 3. Exemptions
    let exemptionsQuery = `
      SELECT 'lab' as type, lr.requested_at as date, p.full_name as patient_name, lt.name as service_name, lr.price as original_price
      FROM visit_lab_requests lr
      JOIN visits v ON lr.visit_id = v.id
      JOIN patients p ON v.patient_id = p.id
      JOIN lab_tests lt ON lr.lab_test_id = lt.id
    ...
    exemptionsQuery += `
      ...
      SELECT 'radiology' as type, rr.requested_at as date, p.full_name as patient_name, rt.name as service_name, rr.price as original_price
      FROM visit_radiology_requests rr
      JOIN visits v ON rr.visit_id = v.id
      JOIN patients p ON v.patient_id = p.id
      JOIN radiology_tests rt ON rr.radiology_test_id = rt.id
    ...
    exemptionsQuery += `
      ...
      SELECT 'clinical' as type, cr.created_at as date, p.full_name as patient_name, cs.name as service_name, cr.price as original_price
      FROM visit_clinical_service_requests cr
      JOIN visits v ON cr.visit_id = v.id
      JOIN patients p ON v.patient_id = p.id
      JOIN clinical_services cs ON cr.service_id = cs.id
